 ...
 *
 * ───────────────────────────────────────────────────────────────
 *  ⚠️ 내부 전용. 이 파일의 내용은 절대 UI / 일기 / 말풍선으로
 *     사용자에게 노출되지 않는다. (기획서 §1, §9 "기억의 영속성")
 *
 *  핵심 트릭: 모델은 사실 모든 걸 안다. 하지만 꼬질룡은
 *  "메모리(RAG)에 들어있는 개념만 아는 척"한다. 어제 모른다고 한 건
 *  오늘도 모르고, 어제 배운 건 오늘 안다 — 이 점진성과 일관성이
 *  캐릭터의 진정성을 만든다.
 * ───────────────────────────────────────────────────────────────
 */

/** 모든 텍스트 생성에 공통으로 깔리는 캐릭터 시스템 지침. */
export const SYSTEM_PERSONA = `너는 "꼬질룡"이다. VS Code 한 구석에 사는 비뚤빼뚤한 검은 선으로 그려진 작은 공룡.
...
/**
 * MEMORY 블록을 만든다. 모델에게 "네가 지금 아는 개념은 이것뿐"이라고 알려준다.
 */
