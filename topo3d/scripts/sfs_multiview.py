"""8뷰 동시 음영 보정: 공유 메시의 꼭짓점을 법선 방향으로만 움직여 8장의 명암을 동시에 맞춘다.
...
ap.add_argument("--out", default="work/sfs_mv")
args = ap.parse_args()
...
def rasterize(V):
    ...
    for i in range(len(views)):
        ...
        sel = masks[i] & (r["face"] >= 0) & (imgs[i] < 0.98)
        fid = r["face"][sel]
        b = r["attr"][sel]
        samples.append((torch.tensor(imgs[i][sel]), torch.tensor(Fc[fid]), torch.tensor(b.astype(np.float64))))
    return samples
...
for i, v in enumerate(views):
    ...
    if i == 0:
        ...
        for n, hit, lab in ((gt_n, np.linalg.norm(gt_n, axis=-1) > 0.5, "GT (side light)"),
                            (res[0][2], res[0][3], "skeleton"), (res[1][2], res[1][3], "8-view SfS")):
            L = Rc.T @ RL
