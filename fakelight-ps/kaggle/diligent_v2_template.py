# 짭광 엔진 v2 (DiLiGenT 물체 하나, 사진 1장). v1 대비 원칙적인 변경만 넣는다:
#   (a) 자동 노출: 마스크 안 선형 밝기 중앙값을 사진 표준 중간 회색(18%)으로 올리고, 원래 흰색이 흰색으로
#       남도록 확장 Reinhard 톤 곡선으로 하이라이트를 눌러 담는다. 이미 밝은 사진은 게인이 1로 잘려 그대로다.
#       생성형 재조명 모델은 제대로 노출된 사진으로 학습됐으니, 그림자 속 형태를 보여줘야 조명을 제대로 바꾼다.
#   (b) IC-Light 회전 없는 세트 시드 2개 (32장)를 SDM-UniPS 한 번에 넣는다.
#   (c) 단안 노멀 추정기(DSINE, MoGe-2)를 앙상블 재료로 같이 저장한다 (오차 성질이 달라서 평균에 유리).
# 경쟁자 RoSE는 v1과 똑같이 노출 안 올린 입력으로 돌린다. 합체와 채점은 로컬(scripts/diligent_eval.py --v2)에서 한다.
...
lin_raw = lin.copy()
lum = lin.mean(2)
gain = max(1.0, 0.18 / (np.median(lum[mask]) + 1e-8))          # 중간 회색 18%까지만 올린다 (내리지는 않음)
Lg = lum * gain
Lt = Lg * (1 + Lg / gain ** 2) / (1 + Lg)                        # 확장 Reinhard, 흰색점 = 원래 흰색
lin = lin * (Lt / (lum + 1e-8))[..., None]
stats["exposure_gain"] = round(float(gain), 3)
srgb = np.where(lin <= 0.0031308, 12.92 * lin, 1.055 * np.clip(lin, 0, None) ** (1 / 2.4) - 0.055)
img = (np.clip(srgb, 0, 1) * 255).round().astype(np.uint8)
img[~mask] = 0
to_srgb8 = lambda l: (np.clip(np.where(l <= 0.0031308, 12.92 * l, 1.055 * np.clip(l, 0, None) ** (1 / 2.4) - 0.055), 0, 1) * 255).round().astype(np.uint8)
img_raw = to_srgb8(lin_raw); img_raw[~mask] = 0                     # v1과 같은 입력 (RoSE용)
...
crop_raw = cv2.resize(np.pad(img_raw, ((pad, pad), (pad, pad), (0, 0)))[y0:y0 + 2 * half, x0:x0 + 2 * half], (F, F), interpolation=cv2.INTER_AREA)
...
Image.fromarray(crop_raw).save(f"{OUT}/input_crop_raw.png")
...
for set_name, use_rot, seed, off in [("ic_rot", True, 1, 0), ("ic_plain", False, 1, 0), ("ic_plain", False, 2, 16)]:
    t0 = time.time()
    for i, ang in enumerate(ANGLES):
        k, d = plan_rotation(ang) if use_rot else (0, ang)
        fg_r = np.ascontiguousarray(np.rot90(fg_gray, k))
        pixels, _ = ic["process"](fg_r, gradient_bg(d, F), prompt, 512, 512, 1, seed, 25, "best quality",
        ...
        Image.fromarray(out).resize((F, F), Image.BICUBIC).save(f"{OUT}/sets/{set_name}__L_{off + i:02d}.png")
    Image.fromarray((cmask * 255).astype(np.uint8)).save(f"{OUT}/sets/{set_name}__mask.png")
    stats[f"{set_name}_s{seed}"] = round(time.time() - t0, 1)
    print(set_name, seed, time.time() - t0, flush=True)
...
# ---------- 5) 같은 사진으로 RoSE (v1과 같은 노출 안 올린 입력) (경쟁자, 공식 test.py와 같은 최소제곱) ----------
try:
    ...
    gray = np.repeat(crop_raw.mean(2, keepdims=True), 3, 2).astype(np.uint8)
...
# ---------- 6) 단안 (pip 설치가 의존성을 건드릴 수 있어 맨 마지막) 노멀 추정기 (앙상블 재료, 원본 출력 그대로 저장) ----------
os.chdir(W)
def run_mono(name, fn):
    try:
        t0 = time.time()
        n = np.asarray(fn(), dtype=np.float32)
        np.save(f"{OUT}/mono_{name}.npy", n)
        stats[f"mono_{name}"] = round(time.time() - t0, 1)
    except Exception as e:
        import traceback; traceback.print_exc()
        stats[f"mono_{name}"] = repr(e)
...
def moge():
    ...
    m = MoGeModel.from_pretrained("Ruicheng/moge-2-vitl-normal").cuda().eval()
    x = torch.tensor(fg_gray / 255.0, dtype=torch.float32, device="cuda").permute(2, 0, 1)
    with torch.no_grad():
        return m.infer(x)["normal"].float().cpu().numpy()

run_mono("dsine", dsine)
run_mono("moge2", moge)
