"""Full 6-view chain: sheet -> calibrated hull -> projective propagation with generator fill,
...
from mf.composite import align_to_guide, restore_and_blend
...
for tgt in order:
    ...
    comp, _ = restore_and_blend(gen_al, guide, known)
    after = preservation(guide, comp, known)
    log(f"view {tgt}: shift {shift}, raw keep edgeF {before['edge_f']:.3f} -> composited {after['edge_f']:.3f}")
    final[tgt] = comp
